...
本節は、上記の調査を別系統 (Claude) で再検証し、未掲載の競合と市場文脈を補った記録である。

再検証は GitHub API、各公式ドキュメント、Web 検索の再取得によって行い、本文の事実主張 (このリポジトリの現状、既存ツール表の各項目、GitHub の停止仕様と API、OpenSSF Scorecard と Swissquote の記述) はすべて一次情報と一致した。
